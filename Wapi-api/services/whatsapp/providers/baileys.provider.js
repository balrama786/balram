...
export default class BaileysProvider extends BaseProvider {
    ...
    async initializeConnection(userId, connectionData = null) {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                if (shouldReconnect) {
                    await delay(5000);
                    await this.initializeConnection(userId, connectionData);
                } else {
                    if (isQRTimeout) {
                        console.log(`QR timeout for WABA ${wabaId}. Breaking loop.`);
                    } else {
                        console.log(`Baileys logged out for WABA ${wabaId}. Cleaning up session...`);
                    }
        ...
        if (params.replyMessageId) {
            sendOptions.quoted = {
                key: {
                    ...
                    remoteJid: jid,
                    fromMe: false // This can be dynamic if needed, but Baileys often finds it by just ID
                },
                message: { conversation: '' } // Baileys needs this structure to quote properly
            };
        }
    ...
    async downloadMedia(wabaId, message, type) {
        try {
            ...
            if (buffer.length === 0) {
                console.error(`Downloaded buffer is empty for ${type} message`);
                return null;
            }
