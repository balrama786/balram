...
class UnifiedWhatsAppService {
  ...
  async sendMessage(userId, params) {
    ...
    if (replyType && replyId) {
      if (replyType === 'ReplyMaterial' || replyType === 'replymaterial' || ['text', 'image', 'video', 'audio', 'document'].includes(replyType)) {
  ...
  async initializeAllConnections() {
    try {
      ...
      console.log(`Auto-initializing ${connections.length} Baileys connections...`);
