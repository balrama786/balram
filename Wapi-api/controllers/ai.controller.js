export const transformMessage = async (req, res) => {
  ...
  try {
    ...
    const validModels = ['gpt-3.5-turbo', 'gpt-4', 'gpt-4-turbo', 'claude-3-opus', 'claude-3-sonnet', 'claude-3-haiku'];
    ...
    if (aiModel.startsWith('gpt')) {
      transformedMessage = await callOpenAI(apiKey, aiModel, prompt);
...
export const suggestReply = async (req, res) => {
  ...
  try {
    ...
    if (aiModel.startsWith('gpt')) {
      suggestedReply = await callOpenAI(apiKey, aiModel, prompt);
    } else if (aiModel.startsWith('claude')) {
