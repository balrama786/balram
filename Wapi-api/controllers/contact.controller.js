...
export const createContact = async (req, res) => {
  try {
    ...
    const tagValidation = await validateTags(tags, req.user.id);
    ...
    const customFieldValidation = await validateCustomFields(custom_fields, req.user.id);
...
export const updateContact = async (req, res) => {
  try {
    ...
    if (updateData.custom_fields) {
      const customFieldValidation = await validateCustomFields(updateData.custom_fields, req.user.id);
      if (!customFieldValidation.isValid) {
        return res.status(400).json({
          success: false,
          message: 'Custom field validation failed',
    ...
    if (updateData.tags) {
      const tagValidation = await validateTags(updateData.tags, req.user.id);
      if (!tagValidation.isValid) {
        return res.status(400).json({
          success: false,
          message: 'Tag validation failed',
