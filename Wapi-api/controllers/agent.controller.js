...
const validateAgentData = (data) => {
    const { name, email, password , country_code , phone } = data;
...
export const createAgent = async (req, res) => {
    try {
        ...
        const { name, email, password , note , country_code , phone , status , is_phoneno_hide } = req.body;

        const validation = validateAgentData({ name, email, password , country_code , phone });
...
export const updateAgent = async (req, res) => {
    try {
        ...
        const validation = validateAgentData({ name, email, password , note , country_code , phone , status });
        if (!validation.isValid) {
            return res.status(400).json({
                success: false,
                message: validation.message
...
export const getAgentById = async (req, res) => {
    try {
        ...
        const agent = await User.findOne(findQuery).lean();
