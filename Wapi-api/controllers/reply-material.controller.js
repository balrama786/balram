...
import { ReplyMaterial, Template, EcommerceCatalog, Sequence } from '../models/index.js';
...
export const getReplyMaterials = async (req, res) => {
    try {
        ...
        const types = ['text', 'audio', 'video', 'document', 'image', 'sticker'];
        const result = {};
        ...
        for (const type of types) {
            ...
            const items = await ReplyMaterial.find(query)
                .select('_id name type content file_path')
                .sort({ createdAt: -1 })
                .skip(skip)
...
export const updateReplyMaterial = async (req, res) => {
    try {
        ...
        if (item.type === 'text' && content) item.content = content;
