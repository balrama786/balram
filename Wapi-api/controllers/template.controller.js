...
export const buildMetaTemplatePayload = (template) => {
  ...
  console.log("template.call_permission" , template.call_permission);
  ...
  if (hasVariables) {
    if (isPositional) {
      ...
      console.log("Positional variables example:", JSON.stringify(bodyComponent.example, null, 2));
    } else {
      bodyComponent.example = {
        body_text_named_params: bodyVariables.map((v) => ({
          param_name: String(v.key),
      ...
      console.log("Named variables example:", JSON.stringify(bodyComponent.example, null, 2));
    }
  }
  ...
  if (template.buttons?.length) {
    ...
    template.buttons.forEach((btn) => {
      ...
      {
        console.log("btn.example" , btn.example);
        quickReplyButtons.push({
          type: "url",
          text: btn.text,
          url: btn.url,
    ...
    const components = (card.components || []).map((comp) => {
      ...
      if (comp.buttons && comp.buttons.length) {
        c.buttons = comp.buttons.map((btn) => {
          ...
          if (Array.isArray(btn.example) && btn.example.length) b.example = btn.example;
          return b;
        });
      }
      return c;
...
10. If you cannot comply exactly, return an empty JSON object {}`;
    ...
    const templateResponse = await commonCallAIModel(aiModel, apiKey, prompt);
