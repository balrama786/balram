...
import { stripe, getStripe } from '../utils/payment-gateway.service.js';
    ...
    try {
        ...
        if (stripeSubscriptionId) {
            const otherActiveSubs = await Subscription.find({
                ...
                stripe_subscription_id: { $nin: [null, '', stripeSubscriptionId] },
                deleted_at: null
            });
            for (const oldSub of otherActiveSubs) {
                if (oldSub.stripe_subscription_id) {
                    try {
                        ...
                        console.error('[Stripe webhook] Failed to cancel old subscription on plan change:', oldSub.stripe_subscription_id, cancelErr);
                    }
                }
                oldSub.status = 'cancelled';
                oldSub.cancelled_at = new Date();
                await oldSub.save();
                console.log('[Stripe webhook] Plan change: cancelled old subscription', oldSub._id);
            }
        }
    } catch (error) {
