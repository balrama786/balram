...
async function getUsageCount(userId, feature, subscription) {
  ...
  switch (feature) {
    ...
    case 'ai_prompts':
      return AIModel.countDocuments({ ...baseQuery, created_by: uid });
    case 'staff':
      return User.countDocuments({ created_by: uid, role: 'agent', deleted_at: null });
    case 'custom_fields':
    ...
    case 'broadcast_messages':
      if (subscription?.usage) {
        const key = `${feature.replace(/s$/, '')}_used`;
        const keyMap = {
          canned_replies: 'canned_replies_used',
          conversations: 'conversations_used',
          broadcast_messages: 'broadcast_messages_used',
        };
        return subscription.usage[keyMap[feature]] ?? 0;
      }
      return 0;
    default:
      return 0;
